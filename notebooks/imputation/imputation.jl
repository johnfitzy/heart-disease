...
## Purpose: 
- Figure out what the best imputation method might be. 
"""

# ╔═╡ 9c90868e-a7e2-4f5e-80c5-b2775e575924
begin
function compare_distributions(feature_name::String,
		before_imputation, after_imputation)
    	
		# Perform unequal variance t-test
    	t_test = UnequalVarianceTTest(before_imputation, after_imputation)
    	p_value = pvalue(t_test)

	println("Feature $feature_name p-value: $p_value")

	# Interpretation
	if p_value < 0.05
    	println("Dataframe's feature: $feature_name IS significantly different (reject H₀).")
	else
    	println("Dataframe's feature: $feature_name is NOT significantly different (fail to reject H₀).")
	end
...
begin
	using Distances, Missings

	# Run the KNN imputation
	matrix_data = Impute.knn(Matrix(df_hung_knn); dist=Euclidean(), k=5, dims=:cols)

	df_hung_knn_after = DataFrame(matrix_data, Symbol.(column_names))
end


# ╔═╡ 026bb1bb-9909-424a-b83f-d82dd743b155
md"""
| Feature Name            | Type        | Percentage Missing (%) |
|-------------------------|-------------|------------------------|
| age                     | Integer     | 0.0                    |
| rest_bp                 | Continuous  | 0.34                   |
| serum_chol              | Continuous  | 0.0                    |
| max_heart_rate          | Continuous  | 0.34                   |
| oldpeak                 | Continuous  | 0.0                    |
| major_vessels           | Continuous  | 98.98                  |
| sex                     | Binary      | 0.0                    |
| fasting_blood_sugar     | Binary      | 2.72                   |
| angina                  | Binary      | 0.34                   |
| chest_pain              | Categorical | 0.0                    |
| electrocardiographic    | Categorical | 0.34                   |
| thal                    | Categorical | 90.48                  |
| slope                   | Integer     | 64.63                  |
| heart_disease           | Integer     | 0.0                    |
"""

# ╔═╡ fed69629-e2a8-4ae9-9ffd-31261bae9390
md"""
## Hungarian Dataset Missing Values Before Imputation
- Here we can see this data see features `slope` (11), `major_vessels` (12) and `thal` (13) have a very high percentage of missing values.
- These features might be unsable, we will see how this goes....
"""

# ╔═╡ 27ca9aff-c088-4362-9cbe-7eaf57b9ab4d
begin
	
	# List with percentages of missing points per feature
	missing_df_hung = missing_datapoints_percent(df_hung)
...
# ╔═╡ 3ac0ffe3-91a7-413e-a278-9288ac5225a5
begin

	

	# Impute continuous values for selected columns using interpolation
	function impute_continuous_values!(df, interp_cols::Vector{String})
    	for col_name in interp_cols
        	df[!, col_name] = Impute.interp(df[!, col_name]) |> Impute.locf() |> Impute.nocb()
    	end
	end


	# Impute categorical and integer types for selected columns using mode
	function impute_categorical_values!(df, substitute_cols::Vector{String})
    	
		# Impute categorical and integer types for selected columns using mode
    	for col_name in substitute_cols
        	df[!, col_name] = Impute.substitute(df[!, col_name], 	statistic=Distributions.mode)
    	end
	end
end

# ╔═╡ 4d4148bd-d6b9-408a-b176-915c9635c614
begin

	#Run basic imputation on Hungarian dataset

	impute_continuous_values!(df_hung_inte, 
		["rest_bp", "serum_chol", "max_heart_rate"])

	impute_categorical_values!(df_hung_inte, 
		["electrocardiographic", "angina", "fasting_blood_sugar"])
end

# ╔═╡ 7aecb25c-3f23-453b-b437-b644784ff388
md"""
# Imputation Interpolation and Substitute
### Interpolation for continuous and substitute for categories and integers
- Note we have not applied the interpolation method on features `slope` (11), `major_vessels` (12) and `thal` (13) yet. 
"""

# ╔═╡ 3d4c6d5a-c127-4c1d-aa53-b7fb460809c0
begin
	
	# List with percentages of missing points per feature
	missing_df_hung_inte = missing_datapoints_percent(df_hung_inte)
	
	bar( 
		missing_df_hung_inte, 
		legend=false, 
		xlabel="Features", 
		ylabel="Percentage Missing", 
...
# ╔═╡ a00efe61-9947-4ada-a07f-ef909c310b1d
md"""
## Compare distributions with t-Test
"""

# ╔═╡ 8761030c-c9ce-46ef-a585-447fca35ab75
# Compare distributions of features before and after imputation
begin
	
	col_names = names(df_hung)

	for i in 1:10	

		original = collect(skipmissing(df_hung[!, col_names[i]]))
		imput = collect(skipmissing(df_hung_inte[!, col_names[i]]))
		
		compare_distributions(col_names[i], original, imput)
	end
end

# ╔═╡ 320b3ef2-7fbe-4c62-b5cc-1cd07418f830
md"""
## Comment
- For features 1 to 10 the distributions before and after imputation with interpolation are NOT significantly statistically different
- What do the features with high amount of missing data look like?


"""

# ╔═╡ 5df0344c-e3d6-4afe-b928-dfd57221cc5a
# Impute some counts 
impute_categorical_values!(df_hung_inte, ["slope", "major_vessels", "thal"])

# ╔═╡ 91c37513-be0f-422a-ae53-577741f87dab
begin

	plot(

		create_hist_plot(df_ds1[!, :slope], 
    		"Thal - DS1.", 
		...
		create_hist_plot(collect(skipmissing(df_hung[!, :slope])), 
    		"Slope - Before Impt.", 
        	"", 
        	"", 
    		"Frequency", 
		...
		create_hist_plot(collect(skipmissing(df_hung_inte[!, :slope])),
    		"Thal - After impt.", 
        	"Density Plot of Thal", 
        	"", 
    		"Frequency", 
...
# ╔═╡ 6441f6a2-8d61-41b2-97ef-b0c1416c4295
md"""
## Comment
- Using `Impute.substitute` with either median or mode simply added a lot of 2's to the `thal` feature
- When comparing this so the `DS1` dataset `thal` feature distribution it makes us think that using the `Impute.substitute` method for a feature with such a large percentage of missing values (90.48%) is **not really that useful.**

"""

# ╔═╡ aeaec49f-d535-426d-8944-e40037c2792f
md"""
# Imputation - KNN
### Interpolation for continuous and substitute for categories and integers
"""

# ╔═╡ 2d8adf51-edc2-4c23-8929-135d6628ab73
df_hung_knn

# ╔═╡ 2669a1ec-1867-482b-b0da-b32ace3f8f5b
begin

	# Round Countinuous values to nearest value
	function round_non_missing_all!(df::DataFrame)
    	for col in names(df)
        	df[!, col] = map(x -> isnan(x) ? missing : ceil(x), df[!, col])
    	end
		return df
	end

	df_hung_knn_after_rnd = round_non_missing_all!(df_hung_knn_after)


end

# ╔═╡ fa942a9a-0f7e-4b15-9291-6735d5a6d086
# clean and flatten it again
clean_and_flatten!(df_hung_knn_after_rnd)

# ╔═╡ db12b687-abe3-4cbd-b1fa-2b62d4f4387b

# Compare dist - ignore feature with large missing data
	for i in 1:10	

		original = collect(skipmissing(df_hung[!, col_names[i]]))
		imput = collect(skipmissing(df_hung_knn_after_rnd[!, col_names[i]]))
...
begin
	...
	plot(
		...
		create_hist_plot(collect(skipmissing(df_hung[!, :thal])), 
    		"Slope - Before Impt.", 
        	"", 
        	"", 
    		"Frequency", 
		...
		create_hist_plot(collect(skipmissing(df_hung_inte[!, :thal])),
    		"Thal - After impt.", 
        	"Density Plot of Thal", 
        	"", 
    		"Frequency", 
...
begin
	...
	# Convert to matrix
	X = Matrix(df_hung_forest)
	
	# Random ForestImputer model
	M = RandomForestImputer(n_trees=20, max_depth=10, recursive_passages=100)

	X_full = fit!(M, X)

	df_hung_forest_impt = DataFrame(X_full, column_names)

	# round off any continuous
	round_non_missing_all!(df_hung_forest_impt)

	# clean and Flatten again. ie floats to ints where required
	clean_and_flatten!(df_hung_forest_impt)
...
begin
	# Compare distributions for features 1 to 10
	for i in 1:10	

		original = collect(skipmissing(df_hung[!, col_names[i]]))
		imput = collect(skipmissing(df_hung_forest_impt[!, col_names[i]]))
		
		compare_distributions(col_names[i], original, imput)
	end
end

# ╔═╡ 8f9e9895-8724-49d6-b3f5-27f22d1e0af4
begin
	...
	plot(
		...
		create_hist_plot(collect(skipmissing(df_hung_forest_impt[!, :thal])),
    		"Thal - After impt.", 
        	"Density Plot of Thal", 
        	"", 
    		"Frequency", 
...
# ╔═╡ b9d15613-fd51-4338-b988-232ed9b0b8a5
begin

	original = collect(skipmissing(df_hung[!, :thal]))

	compare_distributions("thal", original, df_hung_forest_impt.thal)

	compare_distributions("thal", df_ds1.thal, df_hung_forest_impt.thal)
...
md"""
# Below is just the code to imput data in the 3 files and save it again to files
"""

# ╔═╡ 31cacb50-0efd-4e33-8009-91bd2e171a43
# Function to run RandomForest on given dataframes
begin

function impute_random_forest(df::DataFrame)
    ...
    # Create a new DataFrame with imputed data and original column names
    df_imputed = DataFrame(X_full, column_names)

    # Round off any continuous values
    round_non_missing_all!(df_imputed)

    # Clean and flatten the DataFrame (floats to ints where required)
    clean_and_flatten!(df_imputed)
...
begin
	...
	function process_and_impute_dataframe(df::DataFrame, file_name::String)
    	...
    	CSV.write(joinpath(root_path, "data", "DS2_imputed", file_name), df_imputed)
	end

	# Process and impute the `df_hung` dataframe
	process_and_impute_dataframe(df_hung, "imputed.processed.hungarian.csv")

	# Process and impute the `df_clev` dataframe
	process_and_impute_dataframe(df_clev, "imputed.processed.cleveland.csv")

    # Process and impute the `df_swiss` dataframe
	process_and_impute_dataframe(df_swiss, "imputed.processed.swiss.csv")

    # Process and impute the `df_lng_beach` dataframe
	process_and_impute_dataframe(df_lng_beach, "imputed.processed.long_beach.csv")
...
manifest_format = "2.0"
project_hash = "589cc9c6862b7382e4891b15bf833f2ff159dbf2"
