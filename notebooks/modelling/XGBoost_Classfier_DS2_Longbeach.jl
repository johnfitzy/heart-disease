...
# ╔═╡ 4e044e22-d561-494f-a86a-a3ced6c90373
begin
using Pkg
Pkg.activate("../../") 
end
...
# Gradient Boosting: XGBoost Classifier model on DS2 Imputed Long Beach

- This book runs as follows:

1. Clean/Processed as per spec.
2. Initial build of XGBoost Classifier machine
3. Train on partitioned data
4. Performance metrics: Log loss, Accuracy, Confusion Metrics, ROC/AUC, violin plots etc.
...
- The AUC is $auc_u, which is quite low.

Lets see what happens when it is tuned...
"""
...
# ╔═╡ d8cbca94-7860-40e9-b98b-8b94dfbecea1
begin
	using JLSO
	MLJ.save("../../models/XGBoost_Classifier_DS2_LongBeach_Model.jls", t_mach)
end
...
###### As a final comment, tuning significantly improved this model not only in performance but more efficient in computational cost.
"""
