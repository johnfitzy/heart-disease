...
# ╔═╡ 4806d5e0-8412-11ef-0511-dff1122ed9f8
using DataFrames, Plots, MLJ, Combinatorics, CSV, NearestNeighborModels, MLJBase, MLJModels, DecisionTree, MLJDecisionTreeInterface

# ╔═╡ 40cfa982-3423-4ef4-b089-3340bff59838
md"""
# Using Tree Regressor to select best features, range from 1-13 features, and choose the best n features while having the best RMS

- This is a wrapper method of feature selection, using a tree regressor model
- The algorithim runs on looking through feature sets size from 1-13 in an iterative mode, finding out what is the optimal number of give feature size and also the features to select to output the lowest RMS.
"""

# ╔═╡ 6c2f21fd-6e52-47f2-9c92-aac57dc087f3
begin
...
# ╔═╡ b27f8527-8efd-4c55-813b-b5a77890932c
begin
df= CSV.read("data/ds1/heart.dat", header=false,DataFrame)
rename!(df, names(df) .=> column_names)
end

# ╔═╡ 569ec7d3-d173-4377-bccf-718f4d1d95f3
describe(df)

# ╔═╡ 237042a1-5d50-41b1-bfd3-b702c6b67ec4
DecisionTreeRegressor = @load DecisionTreeRegressor pkg=DecisionTree verbosity=0

# ╔═╡ 2b1b890f-522c-4b3c-b852-11137298d5ad
tree_reg=DecisionTreeRegressor()

# ╔═╡ 4a3a792f-4e87-47a2-a307-c195aff38e5b
begin
data = coerce(df, autotype(df, :discrete_to_continuous))	
X = DataFrames.select(data, Not(:heart_disease))
y = data.heart_disease
end

# ╔═╡ 187b481f-7cdc-4de0-acac-3f74042d5fb1
feature_names = names(X)

# ╔═╡ aa371fa6-1433-4db8-8a6f-09deb33b2be2
function evaluate_features(features)
    X_subset = DataFrames.select(X, features)
    mach = machine(tree_reg, X_subset, y)
    evaluate!(mach, resampling=CV(nfolds=15), measure=rms)
end


# ╔═╡ 2dec013c-2cf2-47df-afa6-8ecb894a72c5
begin
results = DataFrame(Subset = String[], RMS = Float64[])
for k in 1:13
    subsets = collect(combinations(feature_names, k))
    for subset in subsets
        rms = evaluate_features(subset).measurement[1]
        push!(results, (join(subset, ", "), rms))
    end
end
end

# ╔═╡ fc64ab4b-2572-4f12-a08b-0bccc2702856
begin
sorted_results = sort(results, :RMS)
top_5_subsets = sorted_results[1:5, :]
println("Top 5 subsets:")
println(top_5_subsets)
end

# ╔═╡ c8e5309e-5260-4170-b4b3-d92dc62e7644
begin
best_subset = results[results.RMS .== minimum(results.RMS), :]
println("Best subset: ", best_subset.Subset[1])
println("RMS: ", best_subset.RMS[1])
end

# ╔═╡ 3ec1f06b-1cf7-4732-99ec-44f0e02f4c8b
md"""
- The result above show the optimal feature selections.
"""
...
manifest_format = "2.0"
project_hash = "a8e92ae75890894fedd7a1f59615d3b8c872f45f"
