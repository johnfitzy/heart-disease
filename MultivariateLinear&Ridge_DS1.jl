...
# ╔═╡ 1e8bac23-1c01-4e4c-ac66-94e66e9de042
data = coerce(df, autotype(df, :discrete_to_continuous));

# ╔═╡ 8f8f77ce-16fb-4f17-b70c-bdf333185ec8
begin
y=data.heart_disease
X= select(data, Not(:heart_disease))
end

# ╔═╡ 08e7f122-6625-44c0-9995-7740717d6801
begin
	r_train, r_test = partition(eachindex(y), 0.8, shuffle=true, rng=13);
	
	reg_lr = LinearRegressor()
	machb = machine(reg_lr, X, y)
	fit!(machb, rows=r_train)
...
begin
	predb = MLJ.predict(machb, rows=r_test)
	round(rms(predb, y[r_test]), sigdigits=4)
end

# ╔═╡ 4d464f95-5b7a-4223-95a7-4c2801fa0916


# ╔═╡ eb50fc5f-0c05-41b1-b740-65c990e878fe
accuracy(predb, y[r_test])
...
begin
	...
	perf = MLJ.evaluate(machperf, X, y, resampling=cv, measure=rms, verbosity=0)
end
	

# ╔═╡ a6e8a141-172b-44b4-8ae1-40e75a227c68


# ╔═╡ 24af3392-fb16-462b-bb13-898639055fe4
begin
mean_rms= mean(perf.measurement)
println("mean RMS: $(round(mean_rms, sigdigits=3))")
end
...
println("standard deviation RMS: $(round(std_rms, sigdigits=3))")
end
...
begin
	...
	perf_r = MLJ.evaluate(machperf_r, X, y, resampling=cv_r, measure=rms, verbosity=0)
end
...
for i in 1:10	
	...
	perflr = MLJ.evaluate(machperf, X, y, resampling=cvLR, measure=rms, verbosity=0)
	
	cvRR = CV(nfolds=10, shuffle=true)
	perfrr = MLJ.evaluate(machperf_r, X, y, resampling=cvRR, measure=rms, verbosity=0)
