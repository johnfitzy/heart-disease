...
# Gradient Boosting: XGBoost model on DS1

- This book runs as follows:

1. Clean and flatten DS1 as per spec.
2. Initial build of XGBoost machine
...
# ╔═╡ 7a8ff20d-fe6b-4eef-8d59-1009224f1dfd
begin
...
# ╔═╡ f60a0d9c-b42b-4246-88aa-b09b5ff1f8b5
clean_and_flatten!(df)

# ╔═╡ 6a57c16f-91b3-4811-8318-8cd2e7b51f5e
df
...
md"""
# XGBoost Regressor Ensemble
"""

# ╔═╡ b70ab936-f767-49ce-95e4-32e8da8b2686
XGboost = @load XGBoostRegressor pkg=XGBoost
...
y_pred= MLJ.predict(mach, dtest)

# ╔═╡ 9a2a9569-f093-4af6-8e7d-dc6c36aba723
...
trtime=@timed begin
	Random.seed!(1)
	perf = MLJ.evaluate!(mach, measure=rms, resampling=CV(nfolds=10, shuffle=true), check_measure=false, verbosity=0)
...
violin(["XGBoost Regression RMS"], XGperf, label=nothing)

# ╔═╡ a0a34df2-0653-4a3b-8f69-aa15a959b4ad
begin
meanXG=mean(XGperf)
stdXG=std(XGperf)
println("mean XG RMS: $(meanXG)")
println("standard deviation of XG RMS: $(stdXG)")
end
...
begin
plotly()
Plots.plot(curve.parameter_values, curve.measurements, ylabel="RMS", xlabel="# num_round", label="")
end

# ╔═╡ 4d4ebd27-4a2b-437f-8b72-d70c93d12334
md"""

- note the run chart above showed about the rms flattens at num_round=8"""

# ╔═╡ 41883cf3-e8f9-4192-9d65-0b3410d96e0b
md"""
## Autotuning the XGBoost Regressor
"""
...
begin
	...
	r2 = range(xgb, :num_round, lower=2, upper=10)
...
begin
t_xgb = XGBoostRegressor(eta=0.3, num_round=10)
...
tutime=@timed begin
	Random.seed!(1)
	t_perf = MLJ.evaluate!(t_mach, measure=rms, resampling=CV(nfolds=10, shuffle=true), check_measure=false, verbosity=0)
...
t_y_pred= MLJ.predict(t_mach, dtest)

# ╔═╡ 1d3a1e8e-9ddf-4429-800f-7d83c9abc548
begin
...
# ╔═╡ cf2fd4e0-7225-428c-bb0c-eab173b9c88a
...
begin
    violin(["Untuned Vs Tuned XGBoost Regressor"], XGperf, side=:left, label="Untuned")
    violin!(["Untuned Vs Tuned XGBoost Regressor"], all_perfT, side=:right, label="Tuned",title="RMS Untuned Vs Tuned side-by-side")
end
...
MannWhitneyUTest(XGperf, all_perfT)

# ╔═╡ 0f241ad4-7b04-4999-bd01-d67898d474d4
md"""
## Summary:

- This book contains a tuned model of XGBoost Regression, trained on partitioned (7:3)DS1, tuned and tested. The machine is autotuned in hyperparameters of number of rounds of boosting (num_round) and learning rate (eta).
...
manifest_format = "2.0"
project_hash = "ca816cef458492a92b2ef0ae915044bb461d13cf"
