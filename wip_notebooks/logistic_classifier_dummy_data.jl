...
# ╔═╡ d37f7198-46c2-4da5-9c84-3a88b6b90951
md"""
## 1) Basic Logistic Regression 
### Implementation
"""

# ╔═╡ 26a15cbb-4419-45a1-b78e-65cf00a6ee34
# Set up test data
begin
...
# ╔═╡ c76f7e87-c1ae-4c0d-9de1-6e607c3e4cc5
# Basic basic example from the documentation
begin

	log_reg_model = LogisticClassifier()

	# Build and fit/train machine
	mach = fit!(machine(log_reg_model, X, y))
...
# ╔═╡ 97bc3df9-fd29-407e-9595-368ccb2e0c65
# Show the params calculated
fitted_params(mach)

# ╔═╡ 77f415bb-c8f1-4cec-9d25-7b29511bc53a
begin
	
	# Convert to normal Julia Matrix
	X_matrix = MLJ.matrix(X)

	scatter(X_matrix[:, 1], 
		X_matrix[:, 2], 
		group=y_pred,
    	title="Logistic Classifier Predictions",
    	xlabel="Feature 1", 
		ylabel="Feature 2",
    	legend=false, markersize=5)
end

# ╔═╡ b9c55177-dba6-40a0-a61c-65a3c640c658
md"""
## 2) Basic Logistic Regression 
### Performance Evalution
"""

# ╔═╡ e8327f55-f389-4d0c-9136-eab42a1857c0
# Cross-validation and ROC curve generation, running again
begin
	cv = CV(nfolds=10)
	clf = LogisticClassifier()
	perf = evaluate(clf, X, y, resampling=cv, measure=auc, verbosity=0)
end


# ╔═╡ 6cf1c233-d951-4251-a9db-1dd1563f564d
begin

	# Probabalistic predictions for ROC curve
	y_pred_prob = predict(mach, X) 
...
# ╔═╡ 46c878f0-6905-40bb-8aec-86fba5dd6041
md"""
## 3) Basic Logistic Regression 
### Statistical Difference Test
"""

# ╔═╡ 54f564e1-33ed-4320-b511-346c3fb1d69e
begin
	
	# Simulate a 10-fold performance array from a different model
	other_model_perf = [0.25, 0.11, 0.87, 0.66, 0.1, 0.1, 0.111, 0.0, 0.85, 0.87]

	# The actual 10 fold performance from the trained model
	perf_per_fold = perf.per_fold[1]

	stat_test = MannWhitneyUTest(perf_per_fold, other_model_perf)
	
	@info "Mann-Whitney U test p-value: $(pvalue(stat_test)) \n= statistically significant difference"

end

# ╔═╡ 8e48e624-0e6d-4180-ad80-8b45954ee558
begin

	violin(["other"], other_model_perf, label=nothing)
	#boxplot!(["k=23"], perfs_k23, label=nothing, alpha=0.2)
	violin!(["this"], perf_per_fold, label=nothing)
	
end

# ╔═╡ 67c98b72-a97f-407e-8ba6-524193700640
md"""
## 4) Basic Logistic Regression 
### Performance Tuning
"""

# ╔═╡ 740c58ea-4c22-4ab1-bd05-9e47d1d0e8d7
begin


	# Range for the strength of the regularizer (lambda)
	r_lambda = range(log_reg_model, :lambda, lower=0.0001, upper=1.0)

	# Range for penalty (regularization type: L1, L2, etc.)
	r_penalty = range(log_reg_model, :penalty, values=[:l1, :l2, :en])

	# Define the tuning model using grid search with 10 resolution points for each parameter
	...
	mach_tune = machine(tuning_model, X, y)
	MLJ.fit!(mach_tune)

	# Report best parameter settings

	rep = report(mach_tune)
...
manifest_format = "2.0"
project_hash = "41a0ef2ab4ad909a9565ce53d7e88f8b1ac28ff2"
