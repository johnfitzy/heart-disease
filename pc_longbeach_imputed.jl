...
md"""
# Processed.va.data  (Long Beach) -Imputed with MLJ.FillImputer (git)
notes:
...
- 199 rows x14 =2786
- missing 696 values
- 24% missing values.(the most)


Here I have imputed it with the MLJ.FillImputer model
...
begin
df1= CSV.read("data/ds2/processed.va.data", DataFrame)
rename!(df1, names(df1) .=> column_names)
end

# ╔═╡ 18969543-35f0-4770-ad13-2a363d612f98
describe(df1)

# ╔═╡ 8b5988ed-2e3b-4633-bc56-58d91c545d2e
for col in names(df1)
    println("Description for column $col:")
    println(describe(df1[!, col]))
end

# ╔═╡ 72053ed6-99e2-466a-933d-91448e4af344
#just to see what missing values are there
begin
positions = Dict()
for col in names(df1)
...
total_question_marks = sum(length, values(positions))
println("Total number of '?' characters: ", total_question_marks)
end

# ╔═╡ f3d8ec71-a9e5-4b65-8e72-75fb1d477bb1
...
# ╔═╡ f3fb039d-72c9-4d49-8308-6a8065f5280f
MLJ.params(imputer)

# ╔═╡ 7daaa17d-027c-4be0-83ed-55b02a14ddd5
...
md"""
The below visualisation show the difference between the imputed histograms and the original. The overlap (when both colours overlap, blue+orange=brown) show closely the imputation followed the original.

- the Fill Imputer method uses the median rather than the mean. """
...
# ╔═╡ 744b2e23-8a8b-4024-aea7-56d12a584224
begin
chol1=histogram(dfte.chol, title="Original Data Vs Imputed- chol", label="Original", alpha=0.5, legend=:topright)
chol2=histogram(dfteNew.chol, color=:red, label="Imputed", alpha=0.5)
plot(chol1, chol2, layout=(2,1))
end
...
md"""
T test indicated these attributes( maxhr, exang, ca, thal) are statiscally different from the original dataset with missing values.
"""
...
manifest_format = "2.0"
project_hash = "2d985964ab71016491341292359b3591f9fc6edc"
